...
We thank the reviewer for this thoughtful comment. We agree that the surrounding
flow field involves tangential motion, shear, and strain that are not directly
represented by the interface-normal velocity and we have expanded the manuscript
to expand the rationale and state the limitations in this choice. In summary,
the DIWECT relies on a scalar weight on each simplex, so the full velocity
vector cannot be used directly. A physically motivated scalar had to be
chosen,\brittany{maybe change this comma to a semi colon?}
\brittany{remove:and} we selected the normal component of the interface normal,
because it is most directly relevant to interface deformation and therefore
breakup.\brittany{maybe point to Section 4.3 here? when reading this, I wanted
the previous sentence to be justified.}
We recognize that this value does not fully describe the flow field, but we believe the trade-off between a compact description and a physically meaningful value is obtained.
...
While we acknowledge that comparing the DIWECT to an unweighted version would be
a useful exercise, regenerating this quantity on the roughly 120,000 extracted
droplets is unfortunately not feasible within the revision period. We have noted
that this is an area for future work.\brittany{if you want something on a
smaller scale, we can likely pull that together. let me know.}
...
Regarding the reconstruction: the weighted Euler characteristic transform is
injective, meaning that with sufficiently dense sampling of directions and
heights, the WECT can be used to reconstruct the original shape and weights. We
have added text to section 3.3 to explicitly state this point.
\brittany{perhaps we can add another sentence here to address the `to what
extent can diwect reconstruct': While the DIWECT does not share this property
(e.g., a chiral object and it's mirror image will have the same DIWECT), it is a
more compact data descriptor and does not require alignment. In other words,
an object and a
rotation of the object would have the same DIWECT but different WECTS (as the
direction matters in the WECT). The benefits of not having to align the objects
and having a smaller descriptor outweigh the potential for the descriptor not to
recover the original object, as the experiments have demonstrated.}
...
We thank the reviewer for identifying these points of confusion and have revised
Section 3.3 for clarity. Specifically, we incorrectly\brittany{I wouldn't
necessarily say this was incorrect. it kinda shows it for both, but only for a
few height values.  we could show it as a whole "timeline" too, if that would be
helpful.}
